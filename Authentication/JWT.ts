...
export default class JWT {
        ...
        app.post("/login",
            ...
            async (req: any, res: any) => {
                ...
                if (req.body.name == undefined && req.body.password == undefined) {
                    res.status(401).json({ message: "No credential Provided" });
                }

                let users = await credential.getAllCredentials();
                let user = users.find(user => { console.dir(user); return user.isUser(req.body.name, req.body.password) });
                ...
                if (user != undefined) {
                    ...
                    res.status(401).json({ message: "password or username is Incorrect", response: "failed" });
                }
            });
    };
    ...
    static authorizeTo(selfAuthorize: boolean, ...authorizedRoles: number[]) {
        return function (req, res, next) {
            ...
            let isSelfReq = selfAuthorize && req.user.id === req.param(`id`, -1);
